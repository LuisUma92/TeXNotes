...
class App(tk.Tk):
    ...
    def __init__(self, network):
        ...
        self.canvas = tk.Canvas(
            self, width=1920, height=1080, bg="white", bd=0, highlightthickness=0
        )
        ...
        for source, dest in self.network.edges:
            ...
            source_id = [
                node_id for node_id, node in self.node_ids.items() if node == source
            ][0]
            dest_id = [
                node_id for node_id, node in self.node_ids.items() if node == dest
            ][0]
    ...
    def convert_coordinates(self, x, y):
        ...
        return (x - min_x) / (range_x) * self.width, (y - min_y) / (
            range_y
        ) * self.height
    ...
    def _add_node_circle(self, node):
        ...
        text_id = self.canvas.create_text(
            x,
            y + self.radius * 2.5,
            text=" ".join([s.capitalize() for s in node.split("_")]),
        )
    ...
    def select_circle(self, event):
        nodes = [
            node
            for node in self.canvas.find_withtag(tk.CURRENT)
...
if __name__ == "__main__":
    ...
    network = nx.relabel_nodes(
        network, {i: note.filename for i, note in enumerate(notes)}
    )
