...
class Helper:
    ...
    renderers = {
        "pdf": ["pdflatex", ["--interaction=scrollmode"]],
        "html": [
            "make4ht",
            ["-um", "draft", "-c", os.path.join("..", "config", "make4ht.cfg"), "-"],
        ],
    }  # {'format': ['command_line_command', ['list', 'of', 'commandline', 'options']]}
    ...
    def rename_recent(n=1):
        ...
        new_reference = (
            input(f"Change reference to [{db_file.reference}]: ") or db_file.reference
        )
    ...
    def sync_md():
        ...
        sb_file_names = {
            os.path.basename(f)[:-3]: "_".join(
                [s for s in os.path.basename(f).split(" ")]
            )[:-3]
            for f in markdown_files
        }
        print(
            "Warning: this may overwrite any files in notes/slipbox that share their filename with a file in notes/md. Do you wish to continue?"
        )
        ...
        for file in markdown_files:
            ...
            if sb_file_names[filename] not in tracked_note_files:
                reference_name = "".join(
                    [w.capitalize() for w in sb_file_names[filename].split("_")]
                )
                note = database.Note(
                    filename=sb_file_names[filename],
                    reference=reference_name,
                    created=datetime.datetime.now(),
                    last_edit_date=datetime.datetime.now(),
                )
        ...
        for file in markdown_files:
            ...
            print(sb_file)
            try:
                edit_delta = os.path.getmtime(file) - os.path.getmtime(
                    os.path.join("notes", "slipbox", f"{sb_file}.tex")
                )
            except FileNotFoundError:
                edit_delta = 1
            ...
            if edit_delta > 0:
                ...
                text = re.sub(
                    regex,
                    lambda m: f"\\excref{{{md_name_to_reference(m)}}}",
                    file_contents,
                )
                ...
                text = re.sub(
                    regex,
                    lambda m: f"\\excref[{m.group(2)}]{{{md_name_to_reference(m)}}}",
                    text,
                )
                ...
                text = re.sub(
                    regex,
                    lambda m: f"\\exhyperref{{{md_name_to_reference(m)}}}{{{m.group(2)}}}",
                    text,
                )
                ...
                text = re.sub(
                    regex,
                    lambda m: f"\\exhyperref[{m.group(2)}]{{{md_name_to_reference(m)}}}{{{m.group(3)}}}",
                    text,
                )
                ...
                process = subprocess.run(
                    [command, *options], input=text.encode(), capture_output=True
                )
    ...
    def render(filename, format="pdf", biber=False):
        ...
        process = subprocess.run(
            [command, *options], input=document.encode(), capture_output=True
        )
        os.chdir("..")
    ...
    def render_updates(format="pdf"):
        ...
        for note in database.Note:
            ...
            if format == "pdf":
                if (
                    note.last_build_date_pdf is None
                    or note.last_edit_date > note.last_build_date_pdf
                ):
                    updated.append(note)
                    run_biber[note] = True
                    # fix reference in
                ...
                if (
                    note.last_build_date_html is None
                    or note.last_edit_date > note.last_build_date_html
                ):
                    updated.append(note)
                    run_biber[note] = True
                    # fix referenced in
    ...
    def force_synchronize():
            ...
            for line in f:
                ...
                if m:
                    ...
                    if filename not in notes:
                        print(
                            f"File {filename} with reference {reference_name} missing from notes. Make new note now? (y/n)"
                        )
                        if Helper.__getyesno():
        ...
        for filename, reference_name in tracked_notes.items():
            ...
            try:
                ...
                try:
                    html_render = datetime.datetime.fromtimestamp(
                        os.path.getmtime(f"html/{filename}.html")
                    )
                ...
                try:
                    pdf_render = datetime.datetime.fromtimestamp(
                        os.path.getmtime(f"pdf/{filename}.pdf")
                    )
        ...
        for note in notes:
            ...
            if note not in tracked_notes:
                print(
                    f"File {note} not tracked by the file documents.tex. Add to the file now? (y/n)"
                )
                if Helper.__getyesno():
        ...
        for line in file:
            new_file.extend(
                (
                    re.sub(
                        r"\\ex(hyper)?(c)?ref(\[([^]]+)\])?\{(.*?)\}(\{(.*?)\})?",
                        lambda m: replace_string(m, md_links),
                        line,
                    )
                ).encode()
            )
    ...
    def export_project(project_folder, texfile=None):
        ...
        try:
            ...
            print(
                "Export already exists, continue? (Warning: This will overwrite any changes you have made to the file {texfile} in {output_dir})"
            )
            if not Helper.__getyesno():
                return
        ...
        with open(input_file, "r") as f:
            for line in f:
                ...
                external_documents = re.finditer(
                    r"\\transclude(\[([^]]+)\])?\{([^}]+)\}", line
                )
        ...
        for note in database.Note:
            ...
            if len(tracked_set) < len(tracked_keys):
                ...
                for key in tracked_set:
                    citation_instances = [
                        c
                        for c in database.Citation.select().where(
                            database.Citation.note == note,
                            database.Citation.citationkey == key,
                        )
                    ]
    ...
    def __update_links(note):
        ...
        tracked = [
            (link.target.note.reference, link.target.label) for link in note.references
        ]
        # add in untracked
        for link in links:
            if link not in tracked:
                try:
                    ...
                    print(
                        f"label in {note.filename} with details {link[0]}, {link[1]} does not exist"
                    )
        ...
        for link in note.references:
            if (link.target.note.reference, link.target.label) not in links:
                print(
                    f"link {(link.target.note.reference, link.target.label)} no longer exists, deleting"
                )
    ...
    def __createnotefile(filename, extension="tex"):
        ...
        try:
            with open(os.path.join(folder, f"{filename}.{extension}"), "r") as f:
                pass
            print(
                f"File {folder}/{filename}.{extension} already exists, skipping copying the template"
            )
            return
        except FileNotFoundError:
            pass
        ...
        with open(os.path.join("template", f"note.{extension}"), "r") as f:
            for i, line in enumerate(f):
                if extension == "tex":
                    file.extend(
                        (
                            re.sub(r"\\title\{(.*?)\}", r"\\title{" + title + "}", line)
                        ).encode()
                    )
                elif extension == "md":
...
def main(args: tuple[str, ...]):
    ...
    try:
        ...
        print("No argument passed, try 'manage help'")
        return
    ...
    try:
        ...
        print(
            f"Unregognised command {args[0]}, try 'help' for a list of availlable commands"
        )
