# src/TexNotes/LatexZettel/database.py
import peewee as pw
...
class Note(BaseModel):
    filename = pw.CharField(
        unique=True
    )  # filename of the note, eg note.tex or subfolder/note.tex
    reference = pw.CharField(
        unique=True
    )  # \externaldocument reference for the note, default value for example_note.tex would be ExampleNote.
...
class Link(BaseModel):
    ...
    source = pw.ForeignKeyField(
        Note, on_delete="CASCADE", backref="references"
    )  # note containing the reference
    target = pw.ForeignKeyField(
        Label, on_delete="CASCADE", backref="referenced_by"
    )  # \excref{target}{target_label}
